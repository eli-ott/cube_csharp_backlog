...
const FamilyTable = () => {
  ...
  useEffect(() => {
    const fetchFamilies = async () => {
      ...
      if (data) setFamilies(data.items);
    };
    fetchFamilies();
  }, [refresh]);
  ...
  const updateFamily = async (familyId: number, newName: string) => {
    if (newName && newName.trim() !== "") {
      ...
      setRefresh(refresh + 1); notify("Mis à jour !", "success")// Rafraîchir la liste après la mise à jour
    } else {
    ...
    <div className="overflow-x-auto p-4 flex flex-col gap-4">
      <div className="actions flex flex-row gap-4">
        <FamilyDialog
          ...
          onFamilyCreated={() => setRefresh(refresh + 1)}
        />
        <button
            ...
            families.map((family) => (
              ...
              >
                ...
                <td className="text-center px-6 py-3 border-b">
                  <button
                    className="mr-2"
                    onClick={(event) => deleteFamily(event, family.familyId)}
                  >
                    <img
                      src={trash}
                      alt="Supprimer"
